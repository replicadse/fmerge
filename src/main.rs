...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cmd.command {
        ...
        | crate::args::Command::Merge { path, pattern } => {
            merge(std::fs::canonicalize(path)?.as_path(), &pattern).await?;
            Ok(())
        },
    }
}
...
mod tests {
    ...
    fn test_merge_mds() {
        assert!(
            "# A\n# B\n# C"
                == exec(r#"cargo run -- merge --path=./test/a.md --pattern="{{ _include \",\" }}""#).unwrap()
        )
    }
}
